...
function TableFooter({ className, ...props }: React.ComponentProps<"tfoot">) {
  return (
    <tfoot
      className={cn("border-t bg-[hsl(var(--muted))] font-medium [&>tr]:last:border-b-0", className)}
      {...props}
    />
  );
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      className={cn(
        "border-b border-[hsl(var(--border))] transition-colors hover:bg-[hsl(var(--accent))]/40 data-[state=selected]:bg-[hsl(var(--accent))]",
        className
      )}
      {...props}
    />
